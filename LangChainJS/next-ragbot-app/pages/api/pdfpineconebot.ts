/*
 * 호출 주소: http://localhost:3000/api/pdfpineconebot
 * PDF 파일 Pinecone Cloud Vector DB 기반 RAG 챗봇 구현
...
import { Pinecone as PineconeClient } from '@pinecone-database/pinecone' // PineconeClient라는 이름으로 Pinecone 객체 참조
...
export default async function handler(
    ...
    try {
        ...
        if (req.method == 'POST') {
            ...
            // ! 임베딩만 Pinecone 벡터 저장소에 저장하도록 변경
            // ! Step3-3: Embedding - Splitting된 문서 내 단어들을 임베딩(백터화) 처리하여, Pinecone 벡터 저장소에 저장
            // 지정한 임베딩 모델을 통해 Chunk data를 개별 Vector로 수치화하고, 수치화된 데이터를 지정한 Cloud Pinecone Vector 전용 저장소에 저장한다.
            ...
            console.log('Pinecone 벡터 저장소 쿼리 검색 결과:', searchResult);
            console.log('Pinecone 벡터 저장소 저장 완료');
