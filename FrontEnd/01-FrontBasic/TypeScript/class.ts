...
// 클래스는 클래스를 상속받아 기능과/속성을 확장(extends)할수 있다.
...
// 클래스는 인터페이스를 상속받으면 반드시 인터페이스의 기능과 속성을 구현(implements)해줘야 한다.
// 인터페이스는 직접적인 기능구현을 하지 않고 형식만 정의해서 해당 형식을 구현하거나 확장할수 있는 방법을 제공한다.
interface Movable {
    speed: number;
    move(): void;
}
...
// 만들어진 객체는 프로그램이 실행되는 컴퓨터의 메모리에 저장되며 이러한 과정을 "인스턴스화"라고 표현한다.
// 클래스는 인스턴화될 떄 클래스내 정의된 생성자 함수가 자동으로 실행되고 해당 함수에 파라미터를 전달할 수 있다.
...
let user = new User("박유경", "1234", "test@test.co.kr");
user.greeting();
...
class AdminUser extends User {
    ...
    constructor(
        ...
        // super메소드는 부모클래스의 생성자 함수를 호출하는 것
        ...
        // 부모클래스에서 protected로 정의되었기에 자식클래스인 AdminUser에서 사용 가능
        this.email = email;
        this.userType = UserType.User;
    }
...
// admin1.changeUserType(UserType.Admin); // changeUserType() 메소드를 통해 부모 클래스의 속성 값을 변경
admin1.changeRoleType(AdminRole.SystemAdmin); //관리자 역할을 바꾸려면
