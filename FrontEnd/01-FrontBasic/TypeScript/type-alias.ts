// 새로운 타입을 만드는데 string 타입과 number타입을 둘다 지원하는 개발자 정의 타입 정의
...
// 객체의 타입을 지정하는 방법
// 회원 데이터 객체의 타입을 미리 정의하고 데이터를 할당
type MemberType = {
    name: string;
    age: number;
...
// 회원 데이터 객체의 구조를 정의하고 값을 할당해서 personData 객체변수를 할당
...
// 함수의 파라메터도 정확히 타입을 지정해서 사용하는 것을 권장
