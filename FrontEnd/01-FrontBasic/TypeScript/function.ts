...
// 항상 타입을 지정한다.
const result: number = add1(10, 10);
...
function greeting1(name: string): string {
    return `일반함수: 안녕하세요. ${name}님!`;
}

// 익명함수
let greeting2 = function(name: string): string {
    return `익명함수: 안녕하세요. ${name}님!`;
}

// 화살표함수
let greeting3 = (name: string): string => {
    return `화살표함수: 안녕하세요. ${name}님!`;
}
...
// plus 함수의 구조를 타입으로 정의
// 함수 타입명 = (매개변수 타입 정의) => 함수 반환값타입;
type CalFunctionType = (a: number, b: number) => number;

// 함수의 구조를 타입으로 정의하고 함수를 구현
...
// calculate()함수에 계산함수를 매개변수로 전달하고,
// 로직처리를 전달된 매개변수 함수를 통해서 계산로직을 처리해서 해당 계산함수의 결과 값을 반환한다.
...
// 함수의 타입을 직접적으로 구현할 수 있다.
function calculate2(a: number, b: number, calFunc: (a:number, b:number) => number) {
    return calFunc(a, b);
}
