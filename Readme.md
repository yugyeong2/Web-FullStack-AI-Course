...
- Query(정보 검색용 질문)
  ...
  - **시맨틱 검색**: 키워드를 숫자/수치(백터)로 변환하여, 숫자와 숫자간의 관계를 통해 코사인 유사도를 계산한다.  
                    단순한 단어가 아니라, 질문의 의미를 파악해서 관련된 정보를 제공한다.  
                    Ex) 추천 시스템에서 시멘틱 검색을 사용한다.

- **:Vector(벡터)**:: 유사도를 수치로 나타낸 것
  - **:코사인 유사도**:: 키워드를 벡터 데이터를 표현해서 두 백터 사이의 각도를 계산(각도가 0에 가까울수록 유사)
  - **:유클리드 거리**:: 백터 사이의 거리를 계산(거리가 가까우면 유사)

- **:Embedding(임베딩)**:
  데이터나 정보를 백터 수치로 변환하는 과정

![alt text](image-9.png)
[FMOps에서 찾는 새로운 기회](https://slownews.kr/94770)

![alt text](image-8.png)
[LangChain: Vectorstore](https://everyshare.co.kr/langchain-vectorstore/)
